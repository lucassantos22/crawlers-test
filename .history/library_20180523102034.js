...
driver.findElement(By.xpath('//table//tr')).then(rows => {
    for(const row of rows){
        row.findElement(By.xpath('.//td')).then(collumns => {
            for(const collumn of collumn)
        })
    }
})


driver.findElement(By.xpath('//aasdas'))
    .then(element => {
        element.sendKeys('dasdasdas');
        element.click();
    });
